...
import com.jipple.sql.catalyst.util.RebaseDateTime.{rebaseGregorianToJulianDays, rebaseGregorianToJulianMicros, rebaseJulianToGregorianDays, rebaseJulianToGregorianMicros}
import com.jipple.sql.errors.ExecutionErrors
import com.jipple.sql.types.{DateType, TimestampType}
import com.jipple.unsafe.types.UTF8String

trait JippleDateTimeUtils {

  final val TimeZoneUTC = TimeZone.getTimeZone("UTC")

  def getZoneId(timeZoneId: String): ZoneId = {
    val formattedZoneId = timeZoneId
      // To support the (+|-)h:mm format because it was supported before Jipple 3.0.
      .replaceFirst("(\\+|\\-)(\\d):", "$10$2:")
      // To support the (+|-)hh:m format because it was supported before Jipple 3.0.
  ...
  /**
   * Converts the timestamp to milliseconds since epoch. In Jipple timestamp values have microseconds
   ...
   *       with Jipple 2.4 and earlier versions. The goal of the shift is to get a local date derived
   ...
   *
   * Note: The date is shifted by the offset of the default JVM time zone for backward compatibility
   *       with Jipple 2.4 and earlier versions.
   *
...
object JippleDateTimeUtils extends JippleDateTimeUtils
