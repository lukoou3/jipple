...
import static com.jipple.unsafe.Platform.BYTE_ARRAY_OFFSET;
...
public final class UnsafeRow extends InternalRow implements Externalizable, KryoSerializable {
  ...
  public static boolean isFixedLength(DataType dt) {
    ...
    PhysicalDataType pdt = PhysicalDataType.of(dt);
  ...
  public void pointTo(Object baseObject, long baseOffset, int sizeInBytes) {
    ...
    if (baseObject == null) {
      throw new UnsupportedOperationException("not support non-heap memory");
    }
  ...
  public void setDecimal(int ordinal, Decimal value, int precision) {
    ...
    if (precision <= Decimal.MAX_LONG_DIGITS) {
      ...
      if (value == null || !value.changePrecision(precision, value.scale())) {
        ...
        final BigInteger integer = value.toBigDecimal().unscaledValue();
  ...
  public Decimal getDecimal(int ordinal, int precision, int scale) {
    ...
    if (precision <= Decimal.MAX_LONG_DIGITS) {
      ...
      return new Decimal(javaDecimal, precision, scale);
    }
  }
