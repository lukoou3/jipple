...
class CodeBlockSuite extends AnyFunSuite {

  test("Block interpolates string and ExprValue inputs") {
    // VariableValue代表一个本地变量
    // 创建一个boolean类型的本地变量
    val isNull: VariableValue = JavaCode.isNullVariable("expr1_isNull")
    val stringLiteral: String = "false"
    val code: Block = code"boolean $isNull = $stringLiteral;"
    assert(code.toString == "boolean expr1_isNull = false;")
  }

  test("Literals are folded into string code parts instead of block inputs") {
    val value: VariableValue = JavaCode.variable("expr1", IntegerType)
    val intLiteral = 1
    val code: Block = code"int $value = $intLiteral;"
