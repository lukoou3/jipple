...
import com.jipple.Logging
import com.jipple.sql.SQLConf
import com.jipple.sql.catalyst.InternalRow
import com.jipple.sql.catalyst.encoders.HashableWeakReference
import com.jipple.sql.catalyst.expressions._
import com.jipple.sql.catalyst.expressions.codegen.Block._
import com.jipple.sql.catalyst.types._
import com.jipple.sql.catalyst.util.{ArrayData, MapData, SQLOrderingUtil, UnsafeRowUtils}
import com.jipple.sql.catalyst.util.DateTimeConstants.NANOS_PER_MILLIS
import com.jipple.sql.errors.QueryExecutionErrors
import com.jipple.sql.types._
import com.jipple.unsafe.Platform
import com.jipple.unsafe.types._
import com.jipple.util.{NonFateSharingCache, ParentClassLoader, Utils}
...
class CodegenContext extends Logging {
  ...
  val partitionClosureStatements: mutable.ArrayBuffer[String] = mutable.ArrayBuffer.empty
  ...
  def addPartitionClosureStatement(statement: String): Unit = {
    partitionClosureStatements += statement
  }

  def closePartition(): String = {
    partitionClosureStatements.mkString("\n")
  }
  ...
  def genComp(dataType: DataType, c1: String, c2: String): String = dataType match {
    ...
    case BinaryType => s"com.jipple.unsafe.types.ByteArray.compareBinary($c1, $c2)"
    case NullType => "0"
...
object CodeGenerator extends Logging {
  ...
  private var _compileTime = 0L

  // Returns the total compile time of Java source code in nanoseconds.
  // Visible for testing
  def compileTime: Long = _compileTime

  // Reset compile time.
  // Visible for testing
  def resetCompileTime(): Unit = _compileTime = 0L
  ...
  def compile(code: CodeAndComment): (GeneratedClass, ByteCodeStats) = try {
    val classLoaderRef = new HashableWeakReference(Utils.getContextOrjippleClassLoader)
  ...
  private[this] def doCompile(code: CodeAndComment): (GeneratedClass, ByteCodeStats) = {
    ...
    val parentClassLoader = new ParentClassLoader(Utils.getContextOrjippleClassLoader)
  ...
  private def logGeneratedCode(code: CodeAndComment): Unit = {
    val maxLines = 1000 // SQLConf.get.loggingMaxLinesForCodegen
    if (Utils.isTesting) {
  ...
  private val cache = {
    ...
    NonFateSharingCache(loadFunc, 100/*SQLConf.get.codegenCacheMaxEntries*/) // 相同的代码会缓存, 改为0后相同的代码会生成两个类，类名一样但是实际不是一个类
  }
