...
import com.jipple.RippleArithmeticException
import com.jipple.sql.catalyst.InternalRow
import com.jipple.sql.catalyst.analysis.TypeCheckResult
import com.jipple.sql.catalyst.analysis.TypeCheckResult.DataTypeMismatch
import com.jipple.sql.catalyst.expressions.codegen._
import com.jipple.sql.catalyst.expressions.codegen.Block._
import com.jipple.sql.catalyst.trees.{SQLQueryContext, TreeNodeTag}
import com.jipple.sql.catalyst.trees.TreePattern._
import com.jipple.sql.catalyst.types.{PhysicalFractionalType, PhysicalIntegralType, PhysicalNumericType}
import com.jipple.sql.catalyst.util._
import com.jipple.sql.catalyst.util.DateTimeConstants._
import com.jipple.sql.catalyst.util.DateTimeUtils._
import com.jipple.sql.catalyst.util.IntervalUtils.{dayTimeIntervalToDecimal, dayTimeIntervalToInt, dayTimeIntervalToLong, yearMonthIntervalToInt}
import com.jipple.sql.errors.{QueryErrorsBase, QueryExecutionErrors}
import com.jipple.sql.internal.SqlApiConfHelper
import com.jipple.sql.types._
import com.jipple.unsafe.types.UTF8String
import com.jipple.unsafe.types.UTF8String.{IntWrapper, LongWrapper}
...
object Cast extends QueryErrorsBase {
   ...
   *   RW  = Row type
   *
   * Ripple's ANSI mode follows the syntax rules, except it specially allow the following
...
case class Cast(
    ...
    evalMode: EvalMode.Value = EvalMode.LEGACY)
  ...
  def this(child: Expression, dataType: DataType, timeZoneId: Option[String]) =
    this(child, dataType, timeZoneId, EvalMode.LEGACY)

  def this(child: Expression, dataType: DataType) =
    this(child, dataType, None, EvalMode.LEGACY)
  ...
  private def typeCheckFailureInCast: DataTypeMismatch = evalMode match {
    case EvalMode.ANSI =>
      Cast.typeCheckFailureMessage(child.dataType, dataType,
        Some(SqlApiConfHelper.ANSI_ENABLED_KEY -> "false"))
    case EvalMode.TRY =>
  ...
  // In ANSI mode, Ripple always use plain string representation on casting Decimal values
  ...
  // Whether Ripple SQL can evaluation the try_cast as the legacy cast, so that no `try...catch`
  ...
  private[this] def castToDateCode(
    ...
    from match {
          ...
          if (ansiEnabled) {
            ...
            code"""
              scala.Option<Integer> $intOpt =
                com.jipple.sql.catalyst.util.DateTimeUtils.stringToDate($c);
              if ($intOpt.isDefined()) {
      ...
      case TimestampType =>
        ...
        (c, evPrim, evNull) =>
          code"""$evPrim =
            com.jipple.sql.catalyst.util.DateTimeUtils.microsToDays($c, $zid);"""
      case TimestampNTZType =>
        (c, evPrim, evNull) =>
  ...
  private[this] def castToTimestampCode(
        ...
        if (ansiEnabled) {
          ...
          code"""
            scala.Option<Long> $longOpt =
              com.jipple.sql.catalyst.util.DateTimeUtils.stringToTimestamp($c, $zid);
    ...
    case DateType =>
      ...
      (c, evPrim, evNull) =>
        code"""$evPrim =
          com.jipple.sql.catalyst.util.DateTimeUtils.daysToMicros($c, $zid);"""
    case TimestampNTZType =>
      val zoneIdClass = classOf[ZoneId]
      val zid = JavaCode.global(
        ctx.addReferenceObj("zoneId", zoneId, zoneIdClass.getName),
  ...
  private[this] def castFractionToIntegralTypeCode(
    ...
    val fromDt = ctx.addReferenceObj("from", from, from.getClass.getName)
    val toDt = ctx.addReferenceObj("to", to, to.getClass.getName)
    // When casting floating values to integral types, Ripple uses the method `Numeric.toInt`
      ...
      case StringType =>
        ...
        (c, evPrim, evNull) =>
          val handleNull = if (ansiEnabled) {
            val errorContext = getContextOrNullCode(ctx)
            "throw QueryExecutionErrors.invalidInputInCastToNumberError(" +
              s"com.jipple.sql.types.FloatType$$.MODULE$$,$c, $errorContext);"
          } else {
            s"$evNull = true;"
          }
          code"""
      ...
      case StringType =>
        ...
        (c, evPrim, evNull) =>
          val handleNull = if (ansiEnabled) {
            val errorContext = getContextOrNullCode(ctx)
            "throw QueryExecutionErrors.invalidInputInCastToNumberError(" +
              s"com.jipple.sql.types.DoubleType$$.MODULE$$, $c, $errorContext);"
          } else {
            s"$evNull = true;"
          }
          code"""
...
case class CheckOverflowInTableInsert(child: Expression, columnName: String)
  ...
  override def eval(input: InternalRow): Any = try {
    ...
    case e: RippleArithmeticException =>
      getCast match {
        case Some(cast) =>
  ...
  def doGenCodeWithBetterErrorMsg(ctx: CodegenContext, ev: ExprCode, child: Cast): ExprCode = {
    ...
    val exceptionClass = classOf[RippleArithmeticException].getCanonicalName
    val fromDt =
