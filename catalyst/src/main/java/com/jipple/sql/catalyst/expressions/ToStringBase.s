...
import com.jipple.sql.catalyst.util.{ArrayData, DateFormatter, IntervalStringStyles, IntervalUtils, MapData, RippleStringUtils, TimestampFormatter}
import com.jipple.sql.catalyst.util.IntervalStringStyles.ANSI_STYLE
import com.jipple.sql.types._
import com.jipple.unsafe.UTF8StringBuilder
import com.jipple.unsafe.types.{CalendarInterval, UTF8String}
...
trait ToStringBase { self: UnaryExpression with TimeZoneAwareExpression =>
  ...
  protected final def castToString(from: DataType): Any => Any = from match {
    ...
    case BinaryType if useHexFormatForBinary =>
      acceptAny[Array[Byte]](binary => UTF8String.fromString(RippleStringUtils.getHexString(binary)))
    case BinaryType =>
  ...
  protected final def castToStringCode(
    ...
    from match {
      case BinaryType if useHexFormatForBinary =>
        (c, evPrim) =>
          val utilCls = RippleStringUtils.getClass.getName.stripSuffix("$")
