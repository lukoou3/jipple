...
import static com.jipple.sql.catalyst.expressions.UnsafeArrayData.calculateHeaderPortionInBytes;

/**
 * A helper class to write data into global row buffer using `UnsafeArrayData` format,
 * used by {@link com.jipple.sql.catalyst.expressions.codegen.GenerateUnsafeProjection}.
 */
  ...
  public void write(int ordinal, Decimal input, int precision, int scale) {
    ...
    if (input != null && input.changePrecision(precision, scale)) {
      if (precision <= Decimal.MAX_LONG_DIGITS) {
        ...
        final byte[] bytes = input.toBigDecimal().unscaledValue().toByteArray();
