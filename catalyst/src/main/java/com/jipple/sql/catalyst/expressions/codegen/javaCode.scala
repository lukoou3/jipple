...
import com.jipple.sql.catalyst.trees.{LeafLike, TreeNode}
import com.jipple.sql.types.{BooleanType, DataType}
    ...
    def code(args: Any*): Block = {
      ...
      if (sc.parts.length == 0) {
        ...
        args.foreach {
          ...
          case other => throw new IllegalArgumentException(s"Can not interpolate ${other.getClass.getName}")
        }
