...
queryOrganization
    : (ORDER BY orders=sortItems)?
      (LIMIT limit=expression)?
    ;

queryTerm
    : queryPrimary                                                                       #queryTermDefault
    | left=queryTerm operator=UNION ALL? right=queryTerm                                 #setOperation
    ;
...
primaryExpression
    ...
    | ANY_VALUE '(' expression (IGNORE NULLS)? ')'                                             #any_value
    ...
    | functionName '(' (argument+=expression (',' argument+=expression)*)? ')'
       (FILTER '(' WHERE where=booleanExpression ')')?                                         #functionCall
